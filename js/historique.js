...
function displayMatchs() {
    ...
    container.innerHTML = matchs.map(match => {
        ...
        // Afficher les buteurs s'ils existent
        const buteursHTML1 = displayButeurs(match.equipe1.buteurs, 1);
        const buteursHTML2 = displayButeurs(match.equipe2.buteurs, 2);
...
function displayButeurs(buteurs, equipeNum) {
    if (!buteurs || buteurs.length === 0) {
        return '';
    }
    
    const buteursHTML = buteurs.map(buteur => {
        const joueur = joueurs.find(j => j.id === buteur.joueurId);
        const nom = joueur ? joueur.nom : 'Inconnu';
        const buts = buteur.buts;
        const csc = buteur.csc;
        
        if (csc) {
            return `<span class="buteur-item csc" title="Contre son camp">⚠️ ${nom} (${buts} CSC)</span>`;
        } else {
            const butsEmoji = '⚽'.repeat(Math.min(buts, 5)); // Max 5 emojis
            return `<span class="buteur-item">${butsEmoji} ${nom}${buts > 1 ? ` (×${buts})` : ''}</span>`;
        }
    }).join('');
    
    return `
        <div class="buteurs-section">
            <div class="buteurs-title">⚽ Buteurs:</div>
            <div class="buteurs-list">${buteursHTML}</div>
        </div>
    `;
}
